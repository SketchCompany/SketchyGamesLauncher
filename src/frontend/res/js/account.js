...
$(document).ready(async function(){
    ...
    if(res.status == 1){
        ...
        $("#password").attr("placeholder", "Dein Neues Passwort") // $("#password").val(res.data.password) is not used because of security reasons
        if(!res.data.games) res.data.games = "[]"
        ...
        // notifyCb("Fehlgeschlagen", res.data + "<br>Klicke um dich neu anzumelden.", "error", 5000, function(){
        //     openSite("/login")
        // })
    ...
    $("#password").keyup(function(e){
        const password = $("#password").val()
        if($("#passwordInvalid").hasClass("valid")){
            $("#saveAccount").css("display", "block")
            $("#saveAccount").css("animation", "moveInFromLeft 750ms")
        }
        else{
