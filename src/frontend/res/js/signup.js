...
async function signup(){
	...
	if(status === 2){
		$("#submit").attr("disabled", true).html($("<span>").addClass(["spinner-grow", "spinner-grow-sm"]).attr("role", "status"))
	
		const res = await send("https://api.sketch-company.de/u/check", { user, email }, true)
		console.log(res)
		if(res.status === 1){
			if(res.data === false){
